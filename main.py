...
def inicializar_csv():
    if not os.path.exists(CSV_FILE):
        ...
        with open(CSV_FILE, mode="w", newline="") as file:
            ...
            writer.writerow(["id_vaga", "titulo", "descricao", "organizacao", "data_publicacao", "localizacao"])
...
def deletar_vaga_por_id(id_vaga: int) -> bool:
    ...
    with open(CSV_FILE, "r") as file:
        ...
        for linha in reader:
            if linha and int(linha[0]) == id_vaga:  # Verifica se o ID da linha é igual ao ID fornecido
    ...
    if vaga_encontrada:
        ...
        raise HTTPException(status_code=404, detail=f"Vaga com ID {id_vaga} não encontrada.")
